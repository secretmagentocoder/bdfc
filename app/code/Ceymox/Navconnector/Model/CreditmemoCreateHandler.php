...
class CreditmemoCreateHandler
{
    ...
    public function processCreditMemo(string $creditMemoId)
    {
        try {
            $creditMemo = $this->creditmemoRepository->get($creditmemoId);
            if ($creditMemo->getId()) {               
                $xmlData = $this->createXmlData($creditMemo);
     ...
     * @return string
     */
    public function createXmlData($creditMemo)
    {
        ...
        $incrementId = $this->configProvider->getCreditMemoPrefix().'-'.$creditMemo->getIncrementId();
        ...
        $creditMemoQty = $this->getCreditMemoQty($creditMemo);
        $totalOrderedQty = $order->getTotalQtyOrdered() - $creditMemoQty;
        ...
        $xmlData = sprintf('<CALTestSuites xmlns="urn:microsoft-dynamics-nav/xmlports/x61013">
            ...
            <order_reference_id>%s</order_reference_id>
    ...
    {        
        ...
        foreach ($order->getItems() as $item) {
            ...
            $prodcutData[] =  sprintf('<MagentoOrderLine>
                <entry_id>%s</entry_id>
                <line_no>%s</line_no>
                <store_id>%s</store_id>
                <created_at>%s</created_at>
        ...
        if (!empty($prodcutData)) {
            $prodcutData = implode("", $prodcutData);
        }
        return $prodcutData;
    }
     ...
     * @return string
     */
    public function getCreditMemoQty($creditMemo)
    {
        $creditMemoQty = 0;
        foreach ($creditMemo->getItems() as $item) {
            $creditMemoQty =+ $item->getQty();
        }
        return $creditMemoQty;
    }
}
