...
abstract class DiscountAbstract extends \Magento\SalesRule\Model\Rule\Action\Discount\AbstractDiscount
{
    ...
    private function isCategoryIdMatched($itemCategory, $promoCategories)
    {
        ...
        foreach ($itemCategory as $categoryId) {
            ...
            if (in_array($categoryId, $promoCategories)) {
                $flag = true;
            }
        }
    ...
    private function getApplicableSKUS($rule)
    {
        ...
        $promoCategories = explode(",", $rule->getPromoCats());
        if ($promoCategories) {
            ...
            foreach ($this->cart->getItems() as $currentItem) {
                ...
                if ($isContainId) {
                    if (!in_array($itemCategory, $promoCategories)) {
                        $promotionSKUs[] = $itemCategory;
                    }
                }
            }
        }
        $promotionSKUs = array_filter($promotionSKUs);
    ...
    public function getAvgQty($avgQty, $cartItems, $currentRule)
    {
        ...
        foreach ($cartItems as $cartItem) {
            if (is_array($cartItem[1])) {

                foreach ($cartItem[1] as $cartsingleitem) {
                    if (in_array($cartsingleitem, $currentRule)) {
    ...
    protected function getRuleNCart($rule, $item, $productSet)
    {
        ...
        foreach ($allRules as $rule) {
            ...
            if ($rule->getPromoSkus()) {
                ...
                $ruleArray[$rule->getId()][] = 'PromoSku';
            }

            if ($rule->getPromoCats() && !$rule->getPromoSkus()) {
                $ruleArray[$rule->getId()][] = explode(',', $rule->getPromoCats());
                $ruleArray[$rule->getId()][] = $rule->getSortOrder();
                $ruleArray[$rule->getId()][] = $rule->getDiscountAmount();
                $ruleArray[$rule->getId()][] = 'PromoCategory';
            }
        }
        ...
        foreach ($item->getQuote()->getAllVisibleItems() as $currentItem) {
            ...
            if ($rule->getPromoCats() && !$rule->getPromoSkus()) {
                $currentItemSku = $currentItem->getProduct()->getCategoryIds();
            }
            ...
            $cartItems[$currentItem->getId()][] = $currentItemSku;
            $cartItems[$currentItem->getId()][] = $currentItem->getQty();

            if (in_array($currentItemSku, $promotionSKUs)) {
                $check++;
            }
        }
