...
class Subselect extends \Webkul\SpecialPromotions\Model\Condition\Order\Combine
{
     ...
     * @return $this
     */
    public function loadArray($conditionData, $key = 'conditions')
    {
        $this->setAttribute($conditionData['attribute']);
        $this->setOperator($conditionData['operator']);
        parent::loadArray($conditionData, $key);
        return $this;
    }
     ...
     * @return string
     */
    public function asXml($conditionKey = 'conditions', $itemKey = 'condition')
    {
        $xml = '<attribute>' .
            $this->getAttribute() .
            '</attribute>' .
    ...
    {
        $htmlData = $this->getTypeElement()->getHtml() . __(
        ...
        if ($this->getId() != '1') {
            $htmlData .= $this->getRemoveLinkHtml();
        }
        return $htmlData;
    }
